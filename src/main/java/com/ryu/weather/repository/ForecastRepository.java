...
public interface ForecastRepository extends JpaRepository<ForecastEntity, String> {
    ...
    @Query("SELECT distinct T1 FROM ForecastEntity T1 LEFT JOIN T1.forecastId.location WHERE T1.forecastId.fcstDate = :FcstDate AND T1.forecastId.location.fcstCoordinate = :coordinate")
